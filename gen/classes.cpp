...
DValue *DtoNewClass(Loc &loc, TypeClass *tc, NewExp *newexp) {
  ...
  // custom allocator
    ...
    DFuncValue dfn(newexp->allocator, getLLFunc(newexp->allocator));
  ...
  if (newexp->member) {
    ...
    DFuncValue dfn(newexp->member, getLLFunc(newexp->member), mem);
...
static LLConstant *build_class_dtor(ClassDeclaration *cd) {
  ...
  return llvm::ConstantExpr::getBitCast(
      getLLFunc(dtor), getPtrToType(LLType::getInt8Ty(gIR->context())));
}
